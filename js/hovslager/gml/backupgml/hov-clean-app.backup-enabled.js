(function(){
  ...
  async function runFirmaBackup(){
    ...
    try{
      const {data,error}=await app.sb.functions.invoke('hov-backup', { body:{ mode:'manual', scope:'firma' } });
      if(error){ msg('firmaBackupMsg','Backup feilet: '+(error.message||JSON.stringify(error)),'err'); return; }
      if(data && data.ok){
        const size = data.file_size ? ` (${Math.round(data.file_size/1024)} KB)` : '';
        const extra = data.signed_url ? ` <a href="${esc(data.signed_url)}" target="_blank" rel="noopener">Last ned backup</a>` : '';
        const el=$('firmaBackupMsg');
        if(el) el.innerHTML = `<div class="msg ok">Backup ferdig: ${esc(data.file_path||data.prefix||'')}${esc(size)}.${extra}</div>`;
      } else {
        msg('firmaBackupMsg','Backup svarte uventet: '+esc(JSON.stringify(data||{})),'err');
      }
  ...
  async function loadFirmaBackupLogg(){
    ...
    let q = app.sb.from('hov_backup_log').select('*').order('created_at',{ascending:false}).limit(20);
    if(!app.isSysadm) q = q.eq('firma_id', app.firmaId);
    const {data,error}=await q;
    if(error){ if(el) el.innerHTML='<div class="msg err">Kunne ikke lese backup-logg: '+esc(error.message)+'</div>'; return; }
    const rows=(data||[]).map(b=>`<tr><td>${esc(String(b.created_at||'').replace('T',' ').slice(0,19))}</td><td>${esc(b.status||'')}</td><td>${esc(b.backup_scope||'firma')}</td><td>${esc(b.file_path||b.backup_prefix||'')}</td><td>${b.file_size ? esc(Math.round(Number(b.file_size)/1024)+' KB') : ''}</td><td>${esc(b.error_message||'')}</td></tr>`);
    if(el) el.innerHTML = table(['Tid','Status','Omfang','Fil','Str.','Feil'], rows);
  }
