(function(){
  ...
  function renderBackupOptions(backups){
    ...
    sel.innerHTML = '<option value="">Velg backup</option>' + (backups||[]).map(b=>{
      const t=String(b.created_at||'').replace('T',' ').slice(0,19);
  ...
  async function loadFirmaBackupLogg(){
    ...
    try{
      ...
      const rows=backups.map(b=>{
        ...
        return `<tr><td>${esc(String(b.created_at||'').replace('T',' ').slice(0,19))}</td><td>${esc(b.status||'')}</td><td>${esc(b.backup_scope||'firma')}</td><td>${esc(b.file_path||'')}</td><td>${b.file_size ? esc(Math.round(Number(b.file_size)/1024)+' KB') : ''}</td><td>${dl}</td></tr>`;
      });
