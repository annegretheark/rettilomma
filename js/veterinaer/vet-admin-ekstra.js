...
(function(){
  ...
  function behandlingHtml(d){
    ...
    function kr(v) {
      try { return typeof formaterKr === 'function' ? formaterKr(v) : Number(v || 0).toFixed(2); }
      catch(e) { return Number(v || 0).toFixed(2); }
    }

    function bilderHtml(j) {
      const bilder = j.vet_journal_bilder || j.bilder || [];
      if (!bilder.length) return '<div class="lite">Ingen bilder lagret på denne behandlingen.</div>';
      return `<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">
        ${bilder.map(b => {
          const url = b.bilde_url || b.url || '';
          return `<a href="${esc(url || '#')}" target="_blank" rel="noopener" style="display:block;width:150px;text-decoration:none;color:inherit;">
            <img src="${esc(url)}" alt="${esc(b.bildetekst || b.filnavn || 'Journalbilde')}" style="width:150px;height:105px;object-fit:cover;border:1px solid #374151;border-radius:8px;background:#111827;">
            <span class="lite" style="display:block;margin-top:3px;">${esc(b.bildetekst || b.filnavn || '')}</span>
          </a>`;
        }).join('')}
      </div>`;
    }

    function varerHtml(j) {
      const varer = j.vet_journal_varer || [];
      ...
      return `<div style="margin-top:8px;">
        ...
        <ul style="margin-top:4px;">
          ${varer.map(v => {
            ...
            return `<li>${esc(v.varenavn || v.navn || 'Vare')} - ${esc(antall)} x ${esc(kr(pris))} kr = ${esc(kr(sum))} kr</li>`;
          }).join('')}
        </ul>
      </div>`;
    }
    ...
    function prisHtml(j) {
      const harPris = Number(j.belop_eks_mva || 0) || Number(j.fastpris || 0) || Number(j.timepris || 0) || Number(j.timer || 0) || Number(j.km || 0);
      if (!harPris) return '';
      return `<div style="margin-top:8px;">
        <strong>Pris / kjøring</strong><br>
        <span class="lite">Fastpris: ${esc(kr(j.fastpris || 0))} kr | Time: ${esc(kr(j.timepris || 0))} x ${esc(j.timer || 0)} | Km: ${esc(j.km || 0)} x ${esc(kr(j.km_pris || 0))} | Sum: ${esc(kr(j.belop_eks_mva || 0))} kr eks. mva</span>
      </div>`;
    }

    function detaljHtml(j) {
      return `<div class="vet-ren-beh-detalj" style="padding:10px;margin:4px 0 10px 0;border:1px solid #374151;border-radius:8px;background:#111827;">
        <div style="display:grid;grid-template-columns:110px 1fr;gap:4px 10px;font-size:13px;margin-bottom:8px;">
          <strong>Dato</strong><span>${esc(datoNo(j.dato))}</span>
          <strong>Type</strong><span>${esc(j.type || 'Behandling')}</span>
          <strong>Behandler</strong><span>${esc((typeof finnBehandlerNavnForJournal === 'function' ? finnBehandlerNavnForJournal(j) : '') || '')}</span>
        </div>
        <strong>Journalnotat</strong>
        <div class="lite" style="white-space:normal;margin-top:4px;">${nl(j.notat || 'Ingen journalnotat skrevet.')}</div>
        ${j.medisin_kladd ? `<div style="margin-top:8px;"><strong>Medisin / reseptkladd</strong><div class="lite">${nl(j.medisin_kladd)}</div></div>` : ''}
        ${prisHtml(j)}
        ${varerHtml(j)}
        <div style="margin-top:8px;"><strong>Bilder</strong>${bilderHtml(j)}</div>
      </div>`;
    }
    ...
    return `<div class="vet-ren-behandling">
      ...
      ${linjer.length ? linjer.map(j => {
        const txt = String(j.notat || j.medisin_kladd || '').replace(/\s+/g,' ').trim();
        return `<div class="vet-ren-beh-wrap">
          <button type="button" class="vet-ren-behlinje" onclick="vetToggleInlineBehandling('${esc(j.id)}')" title="Klikk for detaljer">
            <span><strong>${esc(datoNo(j.dato))}</strong></span>
            <span>${esc(j.type || 'Behandling')}</span>
            <span>${esc(txt ? (txt.length > 90 ? txt.slice(0,90) + '...' : txt) : 'Ingen notattekst')}</span>
          </button>
          <div id="vetInlineBehandling_${esc(j.id)}" style="display:none;">${detaljHtml(j)}</div>
        </div>`;
  ...
  window.vetToggleInlineBehandling = function(journalId){
    const el = qs('vetInlineBehandling_' + String(journalId || ''));
    if (!el) return false;
    const skalVises = el.style.display === 'none' || !el.style.display;
    document.querySelectorAll('[id^="vetInlineBehandling_"]').forEach(x => {
      if (x !== el) x.style.display = 'none';
    });
    el.style.display = skalVises ? 'block' : 'none';
    if (skalVises) el.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    return false;
  };
...
async function lastJournalLogg(){
  ...
  try{
    ...
    liste.innerHTML = rader.map(r => {
      ...
      const epost = r.bruker_epost ? ' (' + r.bruker_epost + ')' : '';
      ...
      return '<div class="listekort">' +
        '<strong>' + esc(bruker) + esc(epost) + '</strong>' +
...
document.addEventListener('click',function(e){
 ...
 if(t && t.getAttribute && t.getAttribute('onclick')==="visVetSide('journalLoggSide')"){
   setTimeout(lastJournalLogg,100);
 }
});
...
/* ===== PROD: MENY FOR FAKTURA / OVERSIKT 2026-06-11 =====
   Holder Journaltilgang urørt. Gjenoppretter bare synlighet og klikk for Faktura/Oversikt. */
(function () {
  function erAdminProd() {
    try {
      const rolle = String(window.vetKlinikkRolle || vetKlinikkRolle || '').toLowerCase();
      const epost = String(window.vetInnloggetEpost || vetInnloggetEpost || '').toLowerCase();
      if (window.vetErSystemAdmin === true || typeof vetErSystemAdmin !== 'undefined' && vetErSystemAdmin === true) return true;
      if (epost === 'greknuts@online.no') return true;
      if (rolle === 'admin' || rolle === 'systemadmin' || rolle === 'klinikkadmin') return true;
      if (typeof erKlinikkAdmin === 'function' && erKlinikkAdmin()) return true;
      return false;
    } catch (e) {
      return false;
  ...
  function visEl(el, synlig, display) {
    if (!el) return;
    el.style.display = synlig ? (display || 'inline-block') : 'none';
    el.classList.toggle('skjult', !synlig);
  }

  function gjenopprettOkonomiOgFakturaMeny() {
    const admin = erAdminProd();

    document.querySelectorAll('.vet-bruker-nav,.vet-lagerlogg-nav').forEach(el => visEl(el, true));

    // Faktura skal være tilgjengelig fra veterinærbildet.
    document.querySelectorAll('.vet-faktura-nav').forEach(el => visEl(el, true));

    // Oversikt/økonomi er adminvalg.
    document.querySelectorAll('.vet-admin-nav').forEach(el => visEl(el, admin));
    document.querySelectorAll('.vet-oppsett-nav,.vet-admin-toggle').forEach(el => visEl(el, admin));

    const oversikt = Array.from(document.querySelectorAll('button')).find(b => (b.textContent || '').trim().toLowerCase() === 'oversikt');
    if (oversikt) {
      visEl(oversikt, admin);
      oversikt.onclick = function () {
        if (typeof vetVisSomVeterinaer !== 'undefined') vetVisSomVeterinaer = false;
        if (typeof window.visVetSide === 'function') window.visVetSide('okonomiSide');
        else if (typeof visVetSide === 'function') visVetSide('okonomiSide');
        return false;
      };
    }

    const faktura = Array.from(document.querySelectorAll('button')).find(b => (b.textContent || '').trim().toLowerCase() === 'faktura');
    if (faktura) {
      visEl(faktura, true);
      faktura.onclick = function () {
        if (typeof window.visVetSide === 'function') window.visVetSide('fakturaSide');
        else if (typeof visVetSide === 'function') visVetSide('fakturaSide');
        return false;
      };
    }
  }

  const gammelOppdater = typeof window.oppdaterVetMenySynlighet === 'function'
    ? window.oppdaterVetMenySynlighet
    : (typeof oppdaterVetMenySynlighet === 'function' ? oppdaterVetMenySynlighet : null);

  window.oppdaterVetMenySynlighet = function () {
    let r;
    if (gammelOppdater) r = gammelOppdater.apply(this, arguments);
    gjenopprettOkonomiOgFakturaMeny();
    return r;
  };

  window.addEventListener('load', function () {
    setTimeout(gjenopprettOkonomiOgFakturaMeny, 100);
    setTimeout(gjenopprettOkonomiOgFakturaMeny, 600);
    setTimeout(gjenopprettOkonomiOgFakturaMeny, 1600);
  });
})();
