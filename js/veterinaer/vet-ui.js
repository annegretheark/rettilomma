/* Siste journal/pasient/bilde/logg/testdata-produksjonskode
...
/* ===== PROD: JOURNALTILGANG SOM EGEN SIDE / SKJUL ANDRE SIDER ===== */


/* ===== SLUTT PROD JOURNALTILGANG ===== */

/* ===== PROD: LAGERLOGG-FANE OG JOURNALTILGANG-VISNING ===== */


/* ===== SLUTT PROD: LAGERLOGG-FANE OG JOURNALTILGANG-VISNING ===== */
...
/* Produksjon: menyvalg som skal finnes etter innlasting */
(function(){
  function erAdmin(){
    try { return typeof erKlinikkAdmin === 'function' && erKlinikkAdmin() && !erVetVisningVanlig(); } catch(e) { return false; }
  }
  function vis(el, ok){ if (el) el.style.display = ok ? 'inline-block' : 'none'; }
  function oppdater(){
    document.querySelectorAll('.vet-bruker-nav,.vet-lagerlogg-nav,.vet-faktura-nav').forEach(el => vis(el, true));
    document.querySelectorAll('.vet-admin-nav,.vet-oppsett-nav,.vet-admin-toggle').forEach(el => vis(el, erAdmin()));
    const oppsett = document.getElementById('vetOppsettKnapp');
    if (oppsett) oppsett.onclick = function(ev){ if(ev){ev.preventDefault();ev.stopPropagation();} if (typeof toggleVetOppsettMeny === 'function') toggleVetOppsettMeny(); return false; };
  }
  const gammel = typeof window.oppdaterVetMenySynlighet === 'function' ? window.oppdaterVetMenySynlighet : null;
  if (gammel && !window.__vetProdMenyWrapped) {
    window.__vetProdMenyWrapped = true;
    window.oppdaterVetMenySynlighet = function(){ const r = gammel.apply(this, arguments); oppdater(); return r; };
    try { oppdaterVetMenySynlighet = window.oppdaterVetMenySynlighet; } catch(e) {}
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', oppdater, {once:true}); else oppdater();
