...
async function loggInn() {
  ...
  // Koble innlogget Supabase-bruker til ansattraden første gang de logger inn.
  // Dette fyller hand_ansatt.user_id automatisk basert på e-post.
  try {
    ...
    console.warn("Kunne ikke koble innlogget bruker til ansatt:", e);
  }
...
let ansattRader = [];
  ...
  if (ansattError) {
    ...
    loginMelding.textContent =
      ...
      (ansattError.message || ansattError);
  ...
  if (ansattData && ansattData.firma_id) {
    const firmaId = String(ansattData.firma_id);
  ...
  // Viktig: navigasjonen viser admin-konsollen bare når både
  // window.erAdmin er true OG localStorage.rilAdminModus = "ja".
  // Nye håndverkerkunder ble opprettet med rolle=admin, men denne
  // verdien ble ikke satt ved innlogging. Da ble adminmenyen skjult.
  ...
  // Best effort: sørg for at firmaeier også finnes i hand_ansatt.
  // Hvis RLS stopper dette, får brukeren likevel adminmodus via regelen over.
