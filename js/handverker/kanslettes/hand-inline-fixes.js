...
(function(){
  ...
  function oppdater(){
    ...
    document.querySelectorAll(".greknuts-only").forEach(function(el){
  ...
  function bind(){
    ...
    if (b && b.dataset.handSplitSys !== "1") {
      ...
      b.onclick = function(){ localStorage.setItem("rilSysadminModus","ja"); oppdater(); };
    }
  ...
  document.addEventListener("DOMContentLoaded", function(){ bind(); setTimeout(oppdater, 300); setTimeout(oppdater, 1200); });
  window.addEventListener("load", function(){ bind(); setTimeout(oppdater, 300); setTimeout(oppdater, 1200); });
  document.addEventListener("click", function(ev){
    if (ev.target && ev.target.id === "loginKnapp") {
      setTimeout(function(){ bind(); oppdater(); }, 1000);
      setTimeout(function(){ bind(); oppdater(); }, 2200);
    }
  });
})();/*
