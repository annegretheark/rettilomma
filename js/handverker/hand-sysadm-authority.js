/* Rett i Lomma - strukturert SysAdm authority
   Eneste globale sysadm-kilde: public.hand_sysadm.
   Vanlig firma-admin hentes fra firma_brukere/hand_firma_bruker/hand_ansatt og gir aldri sysadm.
*/
(function(){
  'use strict';
  ...
  async function isGlobalSysadm(email){
    ...
    if(!c) return false;
    try{
      const r = await c.from('hand_sysadm').select('id,epost,aktiv').ilike('epost', email).eq('aktiv', true).limit(1);
