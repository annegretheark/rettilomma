...
(function(){
  ...
  function hideAll(){
    const ids = ['timerSide','jobberSide','fravaerSide','tilbudSide','backupSide','fakturaSide','varerSide','bilerSide','kundeSide','kunderSide','ansattSide','firmaSide','testSide','testpanelSide','lonnPanel','lonnSide','bilBestillingerSide','adminBilBestillinger','adminBilBestillingerPanel','modulerSide','sysadminPanelSide','adminSide'];
  ...
  async function upsertFirmaBrukerAdmin(firma, navn, epost){
    ...
    try{
      const ex = await window.supabaseClient.from('hand_firma_bruker').select('id').eq('firma_id', firma.id).eq('epost', epost).limit(1);
