...
async function opprettFirmaEierAdminRad(firmaRad) {
  if (!window.supabaseClient || !firmaRad || !firmaRad.id) return;

  let authUser = null;
  try {
    const res = await supabaseClient.auth.getUser();
    authUser = res && res.data ? res.data.user : null;
  } catch (e) {}

  const epost = String((authUser && authUser.email) || firmaRad.epost || firmaRad.email || "").trim().toLowerCase();
  if (!epost && !(authUser && authUser.id)) return;

  const rad = {
    firma_id: firmaRad.id,
    epost: epost || null,
    rolle: "admin"
  };
  if (authUser && authUser.id) rad.user_id = authUser.id;

  try {
    let q = supabaseClient.from("hand_firma_bruker").select("id").eq("firma_id", firmaRad.id).limit(1);
    if (authUser && authUser.id) q = q.eq("user_id", authUser.id);
    else q = q.eq("epost", epost);
    const finnes = await q;

    if (!finnes.error && finnes.data && finnes.data.length) {
      await supabaseClient.from("hand_firma_bruker").update(rad).eq("id", finnes.data[0].id);
    } else {
      await supabaseClient.from("hand_firma_bruker").insert([rad]);
    }
  } catch (e) {
    console.warn("Kunne ikke opprette eier/admin i hand_firma_bruker:", e);
  }
}
...
async function lagreFirma() {
  ...
  if (!result.error && result.data && !(eksisterende && eksisterende.id)) {
    await opprettFirmaEierAdminRad(result.data);
  }
...
// RIL HAND FIRMA TENANT FIX: firmaadmin redigerer eget firma, ikke første firma i databasen.
(function(){
