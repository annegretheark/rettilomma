...
function visPdfIVindu(doc, filnavn) {
  try {
    ...
    console.error("Kunne ikke vise PDF i vindu:", e);
    doc.save(filnavn || "faktura.pdf");
  }
}
...
async function lagEnFakturaPdf(
  ...
  doc.setFontSize(10);
  doc.text("Dato", 14, y);
  doc.text("Beskrivelse", 46, y);
  doc.text("Utfører", 106, y);
  doc.text("Antall", 146, y);
  doc.text("Eks mva", 162, y);
  doc.text("MVA", 184, y);
  ...
  for (const linje of fakturaLinjer) {
    ...
    doc.setFontSize(9);
    doc.text(forkortPdfTekst(doc, linje.dato || "", 32), 14, y);
    doc.text(forkortPdfTekst(doc, linje.beskrivelse || "", 54), 46, y);
    doc.text(forkortPdfTekst(doc, linje.utforer || "", 36), 106, y);
    doc.text(fakturaTekst(linje.antall || 0), 146, y);
    doc.text(formatBelop(linje.sumEksMva), 162, y);
    doc.text(formatBelop(linje.mva), 184, y);
  ...
  visPdfIVindu(doc, filnavn);

  // Kopi/test/forhåndsvisning skal bare lage PDF. Ikke opprett ny fakturapost og ikke sperr linjer.
  if (erKopi) {
    return;
  }
...
async function lagFakturaPdf() {
  ...
  // Utlegg hentes per jobbgruppe senere, ikke samlet for hele kunden.
  const direkteUtlegg = [];
    ...
    for (const key of alleNokler) {
      ...
      // RIL FIX: Ta kun med utlegg som er koblet til disse jobbene via timer_id.
      // Ikke ta med alle åpne utlegg på kunden.
      const gruppeUtlegg = fjernDuplikatUtlegg(await hentDirekteFakturaUtlegg(kunde, gruppeTimer));
...
async function lagFakturaKopiFraOkonomi(fakturanr) {
  fakturanr = String(fakturanr || "").trim();
  if (!fakturanr) {
    alert("Mangler fakturanr.");
    return;
  ...
  const { data: fakturaListe, error: fakturaError } = await supabaseClient
    .from("hand_faktura")
    .select("*")
    .eq("fakturanr", fakturanr)
    .limit(1);
  ...
  let kopiTimer = fakturaId ? await hent("hand_time", q => q.eq("faktura_id", fakturaId)) : [];
  kopiTimer = (kopiTimer || []).map(t => {
    const rad = Object.assign({}, t || {});
    const timer = Number(rad.timer || 0);
    const sumTimer = Number(rad.sum_timer || 0);
    let timepris = Number(rad.timepris || 0);
    if (!timepris && timer > 0 && sumTimer > 0) timepris = sumTimer / timer;
    rad.timer = timer;
    rad.timepris = timepris;
    if ((rad.sum === undefined || rad.sum === null || Number(rad.sum) === 0) && sumTimer > 0) rad.sum = sumTimer;
    if (!rad.beskrivelse && !rad.kommentar) rad.beskrivelse = "Timer";
    return rad;
  }).filter(t => {
  ...
  const kopiVarer = await hent("hand_faktura_vare", q => q.eq("fakturanr", fakturanr));
  const kopiUtlegg = await hent("hand_faktura_utlegg", q => q.eq("fakturanr", fakturanr));
  ...
  visPdfIVindu(doc, "fakturakopi_" + tryggFilnavn(fakturanr) + ".pdf");
}
