...
function prosjektFakturatype(prosjekt) {
  const v = String((prosjekt && (prosjekt.fakturatype ?? prosjekt.faktura_type ?? prosjekt.pristype)) || '').toLowerCase().trim();
  return v === 'fastpris' ? 'fastpris' : 'time';
}

function prosjektFastpris(prosjekt) {
  const n = Number(String((prosjekt && (prosjekt.fastpris ?? prosjekt.fast_pris ?? prosjekt.prosjekt_fastpris)) ?? '').replace(',', '.'));
  return Number.isFinite(n) ? n : 0;
}

function erFastprisProsjektForTimer(timerListe) {
  const prosjekt = finnProsjektForTime((timerListe || [])[0] || {});
  return prosjekt && prosjektFakturatype(prosjekt) === 'fastpris' && prosjektFastpris(prosjekt) > 0;
}

function byggFastprisLinje(timerListe) {
  const ref = (timerListe || [])[0] || {};
  const prosjekt = finnProsjektForTime(ref);
  if (!prosjekt) return null;
  const fastpris = prosjektFastpris(prosjekt);
  if (!(prosjektFakturatype(prosjekt) === 'fastpris' && fastpris > 0)) return null;
  const mva = fastpris * MVA_SATS;
  return {
    dato: ref.dato || '',
    beskrivelse: ('Fastpris: ' + hentProsjektTekst(ref)).slice(0, 40),
    utforer: '',
    antall: 1,
    sumEksMva: fastpris,
    mva: mva
...
function hentUtforerNavnForFaktura(time) {
  ...
  const liste = window.handAnsatte || [];
...
async function hentDirekteFakturaUtlegg(valgtKunde, timerListe = []) {
  // RIL FIX: Utlegg skal følge aktuell jobb/time via timer_id.
  // Vi henter derfor ikke alle åpne utlegg på kunden lenger.
  const timerIder = (timerListe || [])
    .map(t => String(t.id || ""))
    .filter(Boolean);
  ...
  let query = supabaseClient
    ...
    .select("*")
    .eq("fakturert", false)
    .in("timer_id", timerIder)
    .order("created_at", { ascending: true });

  const { data, error } = await query;

  if (error) {
    console.error("Feil ved henting av utlegg for aktuelle jobber:", error);
    return [];
  }
...
async function lagEnFakturaPdf(
  ...
  const fastprisLinje = byggFastprisLinje(fakturaTimer);
  const timeLinjer = fastprisLinje ? [fastprisLinje] : byggFakturaLinjer(fakturaTimer, vareMap);
  ...
  if (valg && valg.returnDoc) {
    return { doc, filnavn, fakturanr, summer };
  }

  doc.save(filnavn);

  if (!erKopi && !erForhandsvisning) {
    const { data: fakturaInsertData, error: fakturaInsertError } = await supabaseClient
      .from("hand_faktura")
      .insert({
    ...
    if (fakturaInsertError) {
      console.error("Faktura ble skrevet ut, men fakturaposten ble ikke lagret:", fakturaInsertError);
      alert("Faktura ble skrevet ut, men fakturaposten ble ikke lagret: " + fakturaInsertError.message);
      return;
    }

    const fakturaId = fakturaInsertData?.id || null;
...
async function lagFakturaPdf() {
  ...
  try {
    ...
    const direkteUtlegg = fjernDuplikatUtlegg(
      await hentDirekteFakturaUtlegg(valgtKunde, timerForMaaned)
    );
...
async function byggIkkeFakturertFakturaPdfFraOkonomi(gruppe) {
  ...
  const pdf = await lagEnFakturaPdf(
    ...
    varer,
    utlegg,
    { forhandsvisning: true, returnDoc: true }
  );

  if (!pdf || !pdf.doc) return null;

  return {
    dataUri: pdf.doc.output("datauristring"),
    filnavn: pdf.filnavn || "faktura_forhandsvisning.pdf",
    fakturanr: pdf.fakturanr || "FORHÅNDSVISNING"
  };
}

async function visIkkeFakturertFakturaFraOkonomi(gruppe) {
  const pdf = await byggIkkeFakturertFakturaPdfFraOkonomi(gruppe);
  if (!pdf) return;
  const vindu = window.open();
  if (vindu) {
    vindu.document.write('<iframe style="border:0;width:100%;height:100vh" src="' + pdf.dataUri + '"></iframe>');
    vindu.document.close();
  }
}

window.lagFakturaPdf = lagFakturaPdf;
window.byggIkkeFakturertFakturaPdfFraOkonomi = byggIkkeFakturertFakturaPdfFraOkonomi;
window.visIkkeFakturertFakturaFraOkonomi = visIkkeFakturertFakturaFraOkonomi;

window.lagFakturaKopiPdf = lagFakturaKopiPdf;
...
// RIL FIX: Fakturakopi direkte fra økonomioversikten.
// Den gamle lagFakturaKopiPdf() lette bare etter fakturanr på timer.
// I denne databasen ligger timer koblet via hand_time.faktura_id, mens varer/utlegg bruker fakturanr.
async function lagFakturaKopiFraOkonomi(fakturanr, valg = {}) {
  fakturanr = String(fakturanr || "").trim();
  if (!fakturanr) {
    alert("Mangler fakturanr.");
    return;
  ...
  let fakturaListe = [];
  let fakturaError = null;

  // Vanlig vei: bruk fakturanummer fra hand_faktura.
  ({ data: fakturaListe, error: fakturaError } = await supabaseClient
    .from("hand_faktura")
    .select("*")
    .eq("fakturanr", fakturanr)
    .limit(1));

  // Sikkerhetsnett: hvis en gammel knapp/rad har sendt UUID i stedet for fakturanr,
  // slå opp fakturaen på hand_faktura.id.
  if ((!fakturaListe || !fakturaListe.length) && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(fakturanr)) {
  ...
  const faktura = (fakturaListe || [])[0];
  if (!faktura) {
    alert("Fant ikke faktura " + fakturanr + ".");
    return;
  }

  const fakturaId = faktura.id || null;
  fakturanr = String(faktura.fakturanr || fakturanr || "").trim();
  const kundeId = faktura.kunden_id || faktura.kunde_id || "";

  const hent = async function(tabell, queryBuilder) {
    try {
      const { data, error } = await queryBuilder(supabaseClient.from(tabell).select("*"));
      if (error) {
        console.warn("Kunne ikke hente " + tabell + ":", error.message);
        return [];
      }
      return data || [];
    } catch (e) {
      console.warn("Kunne ikke hente " + tabell + ":", e);
      return [];
    }
  };

  const normaliserNr = v => String(v || "").trim();
  const fakturanrNorm = normaliserNr(fakturanr);
  const fakturaIdNorm = normaliserNr((valg && valg.fakturaId) || fakturaId);

  const hentAlle = async function(tabell) {
    try {
      const { data, error } = await supabaseClient.from(tabell).select("*");
      if (error) {
        console.warn("Kunne ikke hente " + tabell + ":", error.message);
        return [];
      }
  ...
  // Varer og utlegg er lagret med fakturanr tekst. Hent eksakt først,
  // og filtrer deretter lokalt som sikkerhet mot mellomrom/typeforskjeller.
  let kopiVarer = await hent("hand_faktura_vare", q => q.eq("fakturanr", fakturanrNorm));
  let kopiUtlegg = await hent("hand_faktura_utlegg", q => q.eq("fakturanr", fakturanrNorm));

  if (!kopiVarer.length || !kopiUtlegg.length) {
    const alleVarer = !kopiVarer.length ? await hentAlle("hand_faktura_vare") : [];
    const alleUtlegg = !kopiUtlegg.length ? await hentAlle("hand_faktura_utlegg") : [];
    if (!kopiVarer.length) kopiVarer = alleVarer.filter(v => normaliserNr(v.fakturanr || v.faktura_nr || v.fakturanummer) === fakturanrNorm);
    if (!kopiUtlegg.length) kopiUtlegg = alleUtlegg.filter(u => normaliserNr(u.fakturanr || u.faktura_nr || u.fakturanummer) === fakturanrNorm);
  }

  const timerTreff = [];

  // Timer ligger i hand_time.faktura_id (uuid) og skal kobles mot hand_faktura.id.
  if (fakturaIdNorm) {
    timerTreff.push(...await hent("hand_time", q => q.eq("faktura_id", fakturaIdNorm)));
  }

  // Viktig fallback: utlegg har timer_id. Hvis faktura_id mangler/ikke er satt på timer,
  // kan vi likevel hente timene som hører til fakturaen via fakturerte utlegg.
  const timerIderFraUtlegg = Array.from(new Set((kopiUtlegg || [])
    .map(u => u.timer_id)
    .filter(v => v !== undefined && v !== null && String(v).trim() !== "")
    .map(v => String(v))));
  if (timerIderFraUtlegg.length) {
    timerTreff.push(...await hent("hand_time", q => q.in("id", timerIderFraUtlegg)));
  }

  // Sikkerhetsnett for timer som allerede er lastet i nettleseren.
  const lokaleTimer = (window.timer || []).filter(t => {
    const tid = normaliserNr(t.faktura_id || t.fakturaId || t.fakturaid);
    const id = normaliserNr(t.id || t.timer_id);
    return (fakturaIdNorm && tid === fakturaIdNorm) || (id && timerIderFraUtlegg.includes(id));
  });
  timerTreff.push(...lokaleTimer);

  const kopiTimerMap = new Map();
  for (const t of timerTreff) {
    const nokkel = String(t.id || t.timer_id || JSON.stringify(t));
    if (!kopiTimerMap.has(nokkel)) kopiTimerMap.set(nokkel, t);
  }
  let kopiTimer = Array.from(kopiTimerMap.values());

  // Lokal snapshot fra tidspunktet fakturaen ble laget.
  // Brukes bare som fallback når databaseoppslaget ikke gir timer.
  // Dette er viktig fordi hand_time ikke har fakturanr, bare faktura_id.
  let lokalSnapshot = null;
  try {
    lokalSnapshot = JSON.parse(localStorage.getItem("hand_faktura_snapshot_" + fakturanr) || "null");
  } catch (e) {
    lokalSnapshot = null;
  }
  if (lokalSnapshot) {
    if (!kopiTimer.length && Array.isArray(lokalSnapshot.timer)) kopiTimer = lokalSnapshot.timer;
    if (!kopiVarer.length && Array.isArray(lokalSnapshot.varer)) kopiVarer = lokalSnapshot.varer;
    if (!kopiUtlegg.length && Array.isArray(lokalSnapshot.utlegg)) kopiUtlegg = lokalSnapshot.utlegg;
  }
  ...
  if (kundeId) {
    ...
    if (!kunde) {
      const { data: kundeData } = await supabaseClient
        ...
        .eq("id", kundeId)
        .limit(1);
      kunde = (kundeData || [])[0] || null;
    }
  }

  const firma = await hentFirmaData();
  const maaned = String(faktura.dato || faktura.created_at || new Date().toISOString()).slice(0, 7);

  if (kopiTimer.length || kopiVarer.length || kopiUtlegg.length) {
    const pdf = await lagEnFakturaPdf(
      kunde,
      kopiTimer,
      maaned,
      ...
      kopiVarer,
      kopiUtlegg,
      valg && valg.returnDoc ? { returnDoc: true } : {}
    );
    if (valg && valg.returnDoc) return pdf;
    return;
  }

  // Siste fallback: lag en enkel kopi fra selve fakturaraden hvis linjene ikke finnes.
  const jspdfObj = window.jspdf;
  if (!jspdfObj || !jspdfObj.jsPDF) {
    alert("PDF-biblioteket er ikke lastet.");
    return;
  ...
  doc.setFontSize(10);
  doc.text("Fakturanr", 14, y);
  doc.text(String(fakturanr), 55, y);
  y += 7;
  doc.text("Dato", 14, y);
  doc.text(String(faktura.dato || faktura.created_at || "").slice(0, 10), 55, y);
  y += 7;
  doc.text("Kunde", 14, y);
  doc.text(String(kunde?.navn || kundeId || "Kunde"), 55, y);
  y += 18;

  const eks = Number(faktura.eks_mva || 0);
  const mva = Number(faktura.mva || 0);
  const inkl = Number(faktura.inkl_mva || (eks + mva));
  doc.text("Kopi av faktura " + fakturanr, 14, y);
  y += 12;
  doc.text("Sum eks. mva", 120, y);
  doc.text(formatBelop(eks) + " kr", 195, y, { align: "right" });
  y += 7;
  doc.text("MVA", 120, y);
  doc.text(formatBelop(mva) + " kr", 195, y, { align: "right" });
  y += 7;
  doc.setFontSize(12);
  doc.text("Sum inkl. mva", 120, y);
  doc.text(formatBelop(inkl) + " kr", 195, y, { align: "right" });

  if (typeof tegnBrevfotAlleSiderPdf === "function") {
    tegnBrevfotAlleSiderPdf(doc, firma);
  }

  const filnavn = "fakturakopi_" + tryggFilnavn(fakturanr) + ".pdf";
  if (valg && valg.returnDoc) {
    return { doc, filnavn, fakturanr };
  }

  doc.save(filnavn);
}

async function byggFakturertFakturaPdfFraOkonomi(fakturanr, rad = null) {
  const pdf = await lagFakturaKopiFraOkonomi(fakturanr, { returnDoc: true, fakturaId: rad && rad.id ? rad.id : null });
  if (!pdf || !pdf.doc) return null;

  return {
    dataUri: pdf.doc.output("datauristring"),
    filnavn: pdf.filnavn || ("fakturakopi_" + tryggFilnavn(fakturanr) + ".pdf"),
    fakturanr: pdf.fakturanr || fakturanr
  };
}

window.lagFakturaKopiFraOkonomi = lagFakturaKopiFraOkonomi;
window.byggFakturertFakturaPdfFraOkonomi = byggFakturertFakturaPdfFraOkonomi;
