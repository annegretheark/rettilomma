...
(function () {
  ...
  function oppdaterAdminVisning() {
    ...
    const sysadmin = (window.erSystemadmin === true || erSystembruker) && localStorage.getItem("rilSysadminModus") === "ja";
  ...
  function visModulerSide() {
    ...
    const sysadmin = window.erSystemadmin === true;
...
(function(){
  ...
  function forceBilSide(){
    var app=$('appSide'); show(app); if(app){ app.classList.remove('ril-starter'); app.hidden=false; app.style.display=''; }
...
/* PROD 7084: Min bil for ansatte. Samme funksjon som i index, lagt i ordinær navigasjon. */
(function(){
  'use strict';
  if (window.__handMinBilProdNav7084) return;
  window.__handMinBilProdNav7084 = true;
  function $(id){ return document.getElementById(id); }
  function show(el){ if(!el) return; el.hidden=false; el.style.display=''; el.style.visibility='visible'; if(el.classList) el.classList.remove('skjult','hidden','modul-skjult','ril-starter'); if(el.removeAttribute) el.removeAttribute('aria-hidden'); }
  function hide(el){ if(!el) return; el.hidden=true; el.style.display='none'; if(el.classList) el.classList.add('skjult','hidden'); if(el.setAttribute) el.setAttribute('aria-hidden','true'); }
  ...
  function ensureBilerSide(){ var side=$('bilerSide'); if(side) return side; var app=$('appSide')||document.body; side=document.createElement('section'); side.id='bilerSide'; side.className='kort'; side.setAttribute('data-modul','biler'); side.innerHTML='<h2>Min bil / lager</h2><div id="bilMelding" class="melding"></div><div id="bilLagerListe"></div><div id="bilLagerFyllListe"></div>'; app.appendChild(side); return side; }
  function initBiler(){ ['lastBiler','lastBilLager','lastBilerOgBilLager','handLastBiler','fyllAlleBilvalg','fyllBilvalg','fyllBilvalgTrygt','begrensBilLagerForVanligBruker','fyllBilLagerVareValg','fyllVarevalgFraAktivBil'].forEach(function(fn){ if(typeof window[fn]==='function') setTimeout(function(){ try{ window[fn](); }catch(e){ console.warn('Min bil init feilet:',fn,e); } },20); }); }
  function openMinBil(ev){ if(ev){ ev.preventDefault(); ev.stopPropagation(); if(ev.stopImmediatePropagation) ev.stopImmediatePropagation(); } var app=$('appSide'); show(app); if(app){ app.classList.remove('ril-starter','skjult','hidden'); app.hidden=false; app.style.display=''; app.removeAttribute('aria-hidden'); } sideIds.forEach(function(id){ hide($(id)); }); var side=ensureBilerSide(); show(side); var h=$('handSideOverskrift')||document.querySelector('#appSide h1'); if(h) h.textContent='Min bil / fyll lager'; var h2=side.querySelector('h2'); if(h2) h2.textContent='Min bil / lager'; initBiler(); [40,120,300,700].forEach(function(ms){ setTimeout(function(){ show(app); if(app) app.classList.remove('ril-starter'); show(side); },ms); }); return false; }
  ...
  function bind(){ var b=$('visBilerKnapp'); if(!b) return; b.type='button'; b.textContent='Min bil / fyll lager'; b.setAttribute('data-modul','biler'); b.setAttribute('data-role','ansatt-bil'); b.onclick=openMinBil; show(b); }
  document.addEventListener('click', function(e){ var b=e.target&&e.target.closest?e.target.closest('#visBilerKnapp,[data-role="ansatt-bil"]'):null; if(!b) return; return openMinBil(e); }, true);
