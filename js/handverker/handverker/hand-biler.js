...
window.BILER_JS_VERSION = "PROD firma_brukere_ansatte bilfix 7085";
...
/* RIL 7081: Registrer ny bil skal aldri åpne Bestillinger. */
(function(){
  if (window.__RIL_NY_BIL_KNAPP_7081__) return;
  window.__RIL_NY_BIL_KNAPP_7081__ = true;
  function $(id){ return document.getElementById(id); }
  ...
  function visBilerKun(){
    ['timerSide','jobberSide','fravaerSide','tilbudSide','backupSide','fakturaSide','varerSide','kundeSide','kunderSide','ansattSide','ansatteSide','firmaSide','testSide','testpanelSide','lonnPanel','lonnSide','bilBestillingerSide','adminBilBestillinger','adminBilBestillingerPanel','modulerSide','sysadminPanelSide','adminSide'].forEach(function(id){ hide($(id)); });
    show($('appSide')); show($('bilerSide'));
  }
  function nyBilTrygt(e){
    if(e){ e.preventDefault(); e.stopPropagation(); if(e.stopImmediatePropagation) e.stopImmediatePropagation(); }
    visBilerKun();
    if (typeof window.nyBilSkjema === 'function') { try { window.nyBilSkjema(); } catch(err){} }
    else {
      var o=$('bilSkjemaOmrade'), r=$('bilSkjemaRad'), id=$('bilId'), n=$('bilNavn'), reg=$('bilRegnr'), t=$('nyBilTittel'), k=$('lagreBilKnapp');
      show(o); if(r) r.style.display='grid'; if(id) id.value=''; if(n) n.value=''; if(reg) reg.value=''; if(t) t.textContent='Registrer ny bil'; if(k) k.textContent='Lagre ny bil'; if(n) { try{n.focus();}catch(e){} }
    }
    setTimeout(function(){ visBilerKun(); }, 50);
    setTimeout(function(){ visBilerKun(); }, 250);
    return false;
  }
  function bind(){
    var b=$('nyBilKnapp');
    if(!b || b.__rilNyBil7081) return;
    b.__rilNyBil7081=true;
    b.type='button';
    b.removeAttribute('onclick');
    b.onclick=nyBilTrygt;
    b.addEventListener('click', nyBilTrygt, true);
  }
  document.addEventListener('click', function(e){ var b=e.target && e.target.closest && e.target.closest('#nyBilKnapp'); if(b) return nyBilTrygt(e); }, true);
  ...
  window.addEventListener('load', function(){ bind(); setTimeout(bind,500); });
})();
