...
(function () {
  ...
  async function resolveContext() {
    ...
    contextPromise = (async function () {
      ...
      const authRole = low((user && user.user_metadata && (user.user_metadata.rolle || user.user_metadata.role)) || (user && user.app_metadata && (user.app_metadata.rolle || user.app_metadata.role)));
      const role = low(authRole || employee && employee.rolle || window.innloggetRolle || localStorage.getItem('handInnloggetRolle'));
      const isSysadmin = role === 'sysadm' || role === 'sysadmin' || role === 'systemadmin';
  ...
  function renderCustomers(rows) {
    const targets = ['kundeListe','handKundeListe'].map(byId).filter(Boolean); // IKKE sysadm-listen
  ...
  async function loadCustomersLocked() {
    ...
    if (!ctx.email) {
      setMessage('Fant ikke innlogget e-post. Logg ut og inn igjen.');
      window.kunder = [];
      window.handAdminKunder = [];
      renderCustomers([]);
      return [];
    }

    let q = window.supabaseClient.from(CUSTOMER_TABLE).select('*').order('navn', { ascending: true });
    if (!ctx.isSysadmin) {
      if (!ctx.firmaId) {
        setMessage('Innlogget bruker mangler firma_id i hand_ansatt. Ingen kunder vises.');
        window.kunder = [];
        window.handAdminKunder = [];
        renderCustomers([]);
        ['kundeValg','fakturaKundeValg','modulKundeVelger','okonomiKundeValg'].forEach(id => fillSelect(id, [], 'Velg kunde'));
        return [];
      }
    ...
    const r = await q;
    if (r.error) {
      setMessage('Feil ved henting av kunder: ' + r.error.message);
      return [];
    }
    let rows = Array.isArray(r.data) ? r.data : [];
    if (!ctx.isSysadmin) rows = rows.filter(k => sameFirma(k, ctx.firmaId));

    window.kunder = rows;
    window.handAdminKunder = rows;
    if (ctx.isSysadmin) window.firmaer = rows;
    ...
    setMessage(ctx.isSysadmin ? 'Sysadmin: viser alle kunder. Antall: ' + rows.length : 'Kunder låst til firma_id=' + ctx.firmaId + '. Antall: ' + rows.length);
    return rows;
  }
  ...
  function install() {
    ...
    // handLastKundeliste tilhører sysadm-bedrifter. Ikke overskriv den her.
    window.handLastKunderForFirma = loadCustomersLocked;
