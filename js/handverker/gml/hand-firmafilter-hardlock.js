...
(function(){
  ...
  async function isTrueSysadmin(){
    ...
    if (window.erSystemadmin === true || rolle === 'sysadmin' || rolle === 'systemadmin') return true;
    try {
      if (window.supabaseClient && window.supabaseClient.rpc) {
        const r = await window.supabaseClient.rpc('er_systemadmin');
