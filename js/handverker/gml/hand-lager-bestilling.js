...
(function () {
  ...
  function tegnAdminBestillinger(rader) {
    ...
    const aktive = rader.filter(r => String(r.status || "ny") !== "levert");
  ...
  async function utførAdminLevering() {
    try {
      ...
      for (const v of valgte) {
        ...
        try {
          await supabaseClient.from("hand_lagerlogg").insert([{
            type: "lagerbestilling",
            handling: nyStatus === "levert" ? "lagerbestilling_levert" : "lagerbestilling_delvis_levert",
            bil_id: b.bil_id,
            bil_navn: b.bil_navn || null,
            vare_id: b.vare_id,
            varenr: b.varenr || null,
            varenavn: b.varenavn || null,
            antall: faktiskLevering,
            hovedlager_for: lagerFor,
            hovedlager_etter: lagerEtter,
            bruker_id: b.bestilt_av_user_id || null,
            bruker_epost: b.bestilt_av_epost || null,
            bruker_navn: b.bestilt_av_navn || null,
            hentet_av: b.bestilt_av_navn || b.bestilt_av_epost || null
          }]);
        } catch (loggFeil) {
          console.warn("Lagerlogg ble ikke skrevet:", loggFeil);
        }
