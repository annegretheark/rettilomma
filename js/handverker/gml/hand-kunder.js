...
(function () {
  ...
  async function lagreKunde() {
    ...
    ['kundeId','kundeNavn','kundeAdresse','kundeEpost','kundeTelefon','kundeKontaktperson','kundeKontonr','kundeNr','kundeNrVis'].forEach(function (id2) { var el = $(id2); if (el) el.value = ''; });
  ...
  function redigerKunde(id) {
    ...
    [['kundeId', k.id], ['kundeNavn', k.navn], ['kundeAdresse', k.adresse], ['kundeEpost', k.epost || k.email], ['kundeTelefon', k.telefon || k.telefonnr || k.mobil], ['kundeKontaktperson', k.kontaktperson], ['kundeKontonr', k.kontonr], ['kundeNr', kundenr(k)], ['kundeNrVis', kundenr(k)]].forEach(function (p) { var el = $(p[0]); if (el) el.value = s(p[1]); });
  ...
  function nesteProsjektNrForKunde(kundeId) {
    ...
    var count = (window.prosjekter || []).filter(function (p) { return s(p.kunde_id) === s(kundeId); }).length + 1;
    return (kundenr(k) || 'P') + '-' + count;
  }
  ...
  async function lagreProsjektForValgtKunde() {
    ...
    if (r.error && /column|kolonne|does not exist|schema cache/i.test(String(r.error.message || ''))) {
      var data2 = { kunde_id: kundeId, prosjektnr: pn, navn: navn, beskrivelse: bes, aktiv: true, firma_id: (k && k.firma_id) || cx.firmaId || null };
  ...
  function nullstillKundeSkjema() {
    ['kundeId','kundeNavn','kundeAdresse','kundeEpost','kundeTelefon','kundeKontaktperson','kundeKontonr','kundeNr','kundeNrVis'].forEach(function (id) {
      var el = $(id);
      if (el) el.value = '';
    });
    ...
    var nytt = nesteKundenr();
    if (nr) nr.value = nytt;
    msg('Ny kunde. Fyll inn navn og trykk Lagre kunde.');
  }
