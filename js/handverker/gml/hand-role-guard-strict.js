...
(function(){
  'use strict';
  window.__handRoleGuardStrict = 'v31';
  ...
  function isGreknuts(email){ return norm(email) === 'greknuts@online.no'; }
  ...
  async function fetchDbRole(email){
    if(isGreknuts(email)) return 'systemadmin';
    var c = client();
    if(!c || !email) return '';
  ...
  function applyVisibility(){
    writeGlobals();
    document.querySelectorAll('.systemadmin-only, .sysadmin-entry').forEach(function(el){ state.isSys ? show(el) : hide(el); });
  ...
  async function refresh(){
    ...
    if(!dbRole && isSysRole(state.role) && !isGreknuts(state.email)) state.role = '';
    if(isGreknuts(state.email)) state.role = 'systemadmin';
