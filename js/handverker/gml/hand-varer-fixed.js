...
window.handHentFirmaIdTilVareimport = async function handHentFirmaIdTilVareimport() {
  ...
  async function importerVarer(inputId = "importVarerFil") {
    ...
    if (!firmaId) {
      melding("Fant ikke firma_id for innlogget bruker. Kan ikke importere varer.", true);
      return;
    }
    const raderMedFirma = rader.map(r => ({ ...r, firma_id: r.firma_id || firmaId }));

    const { error } = await supabaseClient.from("hand_vare").upsert(raderMedFirma, { onConflict: "varenr" });
