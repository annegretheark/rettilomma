...
(function () {
  ...
  async function guardedLastKunder() {
    ...
    if (!tenant.ids.length && !tenant.emails.length) {
      replaceGlobalArray('kunder', []);
      replaceGlobalArray('prosjekter', []);
      setMsg('Innlogget bruker er ikke koblet til firma. Ingen kunder vises.');
      renderCustomers();
      return;
    }
    ...
    let all = [];
    try {
      const r = await window.supabaseClient.from('hand_kunde').select('*').order('navn', { ascending: true }).limit(5000);
      if (r.error) throw r.error;
      all = Array.isArray(r.data) ? r.data : [];
      debug.push('hand_kunde total=' + all.length);
    } catch (e) {
      debug.push('hand_kunde feil=' + (e.message || e));
    }

    const filtered = filterCustomers(all, tenant);
    debug.push('etter filter=' + filtered.length);
    if (all[0]) debug.push('kundekolonner=' + Object.keys(all[0]).join(','));
    window.handTenantGuardDebug = debug;
    console.log(VERSION + ' kunder', debug);

    replaceGlobalArray('kunder', filtered);

    const ids = filtered.map(k => k.id || k.kunde_id).filter(Boolean).map(norm);
    let projects = [];
    if (ids.length) {
      try {
    ...
    if (!filtered.length) setMsg('Ingen kunder funnet for innlogget firma. Debug: skriv window.handTenantGuardDebug i konsoll.');
    else setMsg('');
    renderCustomers();
  }
