...
window.handHentFirmaIdTilVareimport = async function handHentFirmaIdTilVareimport() {
  ...
  // Viktig: hent firma fra databasen for innlogget bruker først.
  // Ikke stol på aktivFirmaId/hentAktivFirmaId her, fordi de kan peke på feil firma i cache.
  ...
  if (!res.error && res.data && res.data.firma_id) {
    console.log("Vareimport firma_id:", res.data.firma_id, "rolle:", res.data.rolle);
    return res.data.firma_id;
  }

  console.error("Fant ikke admin/eier i hand_firma_bruker for bruker:", userId, res.error || res.data);
  return null;
};
  ...
  function toNumber(value) {
    if (value === undefined || valuString(value).trim() === "") return 0;
    let s = String(value)
      .replaceAll(" ", "")
      .replaceAll("\u00a0", "")
      .replace("kr", "")
  ...
  async function importVarerTilLager() {
    try {
      ...
      const firmaId = await window.handHentFirmaIdTilVareimport();
      if (!firmaId) {
        msg("Fant ikke firma_id for innlogget bruker. Kan ikke importere varer.", true);
        return;
      }
      rows.forEach(r => { if (!r.firma_id) r.firma_id = firmaId; });
      ...
      for (const row of rows) {
        ...
        if (row.varenr) {
          const check = await supabaseClient.from("hand_vare").select("id").eq("varenr", row.varenr).eq("firma_id", row.firma_id).limit(1);
          if (check.error) {
