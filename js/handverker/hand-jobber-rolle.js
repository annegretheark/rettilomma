...
(function () {
  ...
  function jobbStripPrefix(tekst, jobbId) {
    return String(tekst || "").replace("Jobb #" + jobbId + " - ", "");
  }
  ...
  async function oppdaterJobbVarer(jobbId, rad) {
    ...
    el.innerHTML = '<div class="jobb-kompakt-liste">' +
      varer.map(function(v) {
        const safe = jobbLinjeSafeId("vare_", v.id);
        const navn = jobbStripPrefix(v.navn || v.beskrivelse || "", jobbId);
        return '<div class="jobb-kompakt-linje jobb-vare-linje">' +
          '<input class="jobb-kompakt-navn" id="' + esc(safe) + '_navn" type="text" value="' + esc(navn) + '" placeholder="Vare">' +
          '<input class="jobb-kompakt-antall" id="' + esc(safe) + '_antall" type="number" step="0.01" value="' + esc(blankHvisNullTall(v.antall)) + '" placeholder="Ant.">' +
          '<input class="jobb-kompakt-belop" id="' + esc(safe) + '_pris" type="number" step="0.01" value="' + esc(blankHvisNullTall(v.pris)) + '" placeholder="Pris">' +
          '<button type="button" class="jobb-mini-knapp jobb-vare-lagre" data-id="' + esc(v.id) + '" title="Lagre">✓</button>' +
          '<button type="button" class="jobb-mini-knapp jobb-vare-slett secondary" data-id="' + esc(v.id) + '" title="Slett">×</button>' +
        '</div>';
      }).join("") +
      '</div>';
  ...
  async function oppdaterJobbUtlegg(jobbId, rad) {
    ...
    for (const u of utlegg) {
      ...
      const bilagHtml = erKjoring
        ? '<span class="jobb-ingen-bilag">Kjøring har ikke bilag</span>'
        : (bilag.length
          ? '<div class="jobb-bilag-mini">' + bilag.map(function (b) {
              return '<a href="' + esc(b.url) + '" target="_blank" title="Åpne bilag">' +
                '<img src="' + esc(b.url) + '">' +
              '</a>';
            }).join("") + '</div>'
          : '<span class="jobb-ingen-bilag">Ingen bilag</span>');
      const bilagRad = erKjoring ?
        '<div class="jobb-utlegg-bilag-rad jobb-utlegg-bilag-skjult">' + bilagHtml + '</div>' :
        '<div class="jobb-utlegg-bilag-rad">' + bilagHtml +
          '<input id="' + esc(safe) + '_bilag" type="file" accept="image/*,application/pdf" multiple>' +
          '<button type="button" class="jobb-mini-bilag jobb-utlegg-bilag" data-id="' + esc(u.id) + '">+ bilag</button>' +
        '</div>';

      rader.push('<div class="jobb-kompakt-linje jobb-utlegg-linje">' +
        '<input class="jobb-kompakt-type" id="' + esc(safe) + '_type" type="text" value="' + esc(u.type || "") + '" placeholder="Type">' +
        '<input class="jobb-kompakt-navn" id="' + esc(safe) + '_beskrivelse" type="text" value="' + esc(jobbStripPrefix(u.beskrivelse || "", jobbId)) + '" placeholder="Beskrivelse">' +
        '<input class="jobb-kompakt-belop" id="' + esc(safe) + '_belop" type="number" step="0.01" value="' + esc(blankHvisNullTall(u.belop)) + '" placeholder="Kr">' +
        '<button type="button" class="jobb-mini-knapp jobb-utlegg-lagre" data-id="' + esc(u.id) + '" title="Lagre">✓</button>' +
        '<button type="button" class="jobb-mini-knapp jobb-utlegg-slett secondary" data-id="' + esc(u.id) + '" title="Slett">×</button>' +
        bilagRad +
      '</div>');
    }

    el.innerHTML = '<div class="jobb-kompakt-liste">' + rader.join("") + '</div>';
  ...
  function visJobbDetalj(rad) {
    ...
    html.push('<style>' +
      ...
      '.jobb-kompakt-linje{display:grid;grid-template-columns:minmax(58px,.8fr) minmax(90px,1.5fr) 64px 34px 34px;gap:4px;align-items:center;padding:5px;border:1px solid #374151;border-radius:8px;background:#0f172a;}' +
      '.jobb-vare-linje{grid-template-columns:minmax(120px,1fr) 54px 64px 34px 34px;}' +
      '.jobb-kompakt-linje input,.jobb-kompakt-linje select{height:31px;min-height:31px;padding:3px 6px;font-size:13px;border-radius:6px;box-sizing:border-box;width:100%;}' +
      '.jobb-mini-knapp{height:31px;min-height:31px;padding:0!important;font-size:16px;font-weight:800;border-radius:6px;line-height:1;}' +
      '.jobb-utlegg-bilag-rad{grid-column:1/-1;display:flex;gap:5px;align-items:center;min-width:0;margin-top:1px;}' +
      '.jobb-utlegg-bilag-rad input[type=file]{height:28px;font-size:12px;padding:2px;max-width:135px;}' +
      '.jobb-mini-bilag{height:28px;min-height:28px;padding:0 7px!important;font-size:12px;border-radius:6px;white-space:nowrap;}' +
      '.jobb-bilag-mini{display:flex;gap:3px;overflow:hidden;max-width:90px;}' +
      '.jobb-bilag-mini img{width:26px;height:26px;object-fit:cover;border-radius:4px;border:1px solid #374151;display:block;}' +
      ...
      '.jobb-kompakt-skjema h4{font-size:14px;margin-bottom:6px!important;}' +
      '.jobb-kompakt-skjema .rad{gap:6px!important;}' +
      ...
      '@media(max-width:700px){.jobb-bilde-grid{grid-template-columns:repeat(auto-fill,minmax(44px,1fr));gap:4px}.jobb-bilde-mini img{height:44px;border-radius:6px}.jobb-kompakt-linje{grid-template-columns:58px minmax(82px,1fr) 58px 30px 30px;gap:3px;padding:4px;}.jobb-vare-linje{grid-template-columns:minmax(100px,1fr) 48px 58px 30px 30px;}.jobb-kompakt-linje input{font-size:12px;padding:2px 5px;height:29px;min-height:29px;}.jobb-mini-knapp{height:29px;min-height:29px;font-size:15px;}.jobb-utlegg-bilag-rad input[type=file]{max-width:115px;font-size:11px;}.jobb-mini-bilag{font-size:11px;padding:0 5px!important;}.jobb-bilag-mini{max-width:62px}.jobb-bilag-mini img{width:23px;height:23px;}}' +
    '</style>');
    ...
    html.push('<div class="jobb-kompakt-skjema" style="margin-top:10px; padding:12px; border:1px solid #374151; border-radius:10px; background:#111827;">');
