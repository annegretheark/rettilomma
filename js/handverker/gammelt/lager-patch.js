...
/* Rett i Lomma - trygg lagerpatch v7300
   Legger til import til hovedlager og fylleliste uten å endre original varer.js/biler.js.
*/
(function () {
  "use strict";
  ...
  function ensureBilUi() {
    ...
    // Tittel + hidden id for original biler.js
  ...
  async function importVarerTilLager() {
    try {
      ...
      for (const row of rows) {
        ...
        if (row.varenr) {
          const check = await supabaseClient.from("varer").select("id").eq("varenr", row.varenr).limit(1);
          if (check.error) {
        ...
        if (existing && existing.id) {
          res = await supabaseClient.from("varer").update(row).eq("id", existing.id).select("id").single();
        } else {
          res = await supabaseClient.from("varer").insert([{ id: uuid(), ...row }]).select("id").single();
        }
  ...
  async function hentOgTegnFylleliste() {
    ...
    const res = await supabaseClient.from("varer").select("*").limit(1000);
