...
(function(){
  ...
  async function handFinnMittFirmaId(){
    ...
    try{
      ...
      let q = window.supabaseClient.from('ansatte').select('firma_id').limit(1);
    ...
    try{
      const {data,error}=await window.supabaseClient.from('ansatte').select('firma_id').ilike('epost', email).limit(1).maybeSingle();
      if(!error && data?.firma_id) return data.firma_id;
  ...
  async function hentAktivFirmaRadFix(){
    ...
    let q = window.supabaseClient.from('firma').select('*');
    if(id) q = q.eq('id', id);
  ...
  async function lagreFirmaFix(){
    ...
    if(!eksisterende?.id){ if(melding) melding.textContent='Fant ikke firma å lagre på.'; return; }
    if(Object.keys(firma).length===0){ if(melding) melding.textContent='Ingen nye firmaopplysninger å lagre.'; return; }
    const {data,error}=await window.supabaseClient.from('firma').update(firma).eq('id', eksisterende.id).select().maybeSingle();
    if(error){ console.error(error); if(melding) melding.textContent='Feil ved lagring av firma: '+error.message; return; }
    window.firmaData=data||{}; window.firma=window.firmaData; window.aktivFirmaId=window.firmaData.id||null;
    if(melding) melding.textContent='Firma lagret i databasen';
    await lastFirmaFix();
  }
