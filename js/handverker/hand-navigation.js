...
(function () {
  ...
  function handleButton(id) {
    ...
    if (!fn) return false;
    fn();
    [0,25,100,300,700].forEach(function(ms){ setTimeout(function(){ closeAdminMenu(true); }, ms); });
    return true;
  }
  ...
  function globalCaptureHandler(event) {
    ...
    if (adminBtn) {
      ...
      if (event.type === 'pointerdown' || event.type === 'mousedown' || event.type === 'touchstart') {
        ...
        toggleAdminMenu();
      } else if (Date.now() - lastAdminPointerOpen < 650) {
        return false;
      } else {
        toggleAdminMenu();
      }
  ...
  window.addEventListener('pointerdown', globalCaptureHandler, true);
  window.addEventListener('mousedown', globalCaptureHandler, true);
  window.addEventListener('touchstart', globalCaptureHandler, true);
  ...
  document.addEventListener('click', function(event) {
    ...
    if (adminBtn) {
      ...
      oppdaterAdminVisning();
      if (Date.now() - lastAdminPointerOpen >= 650) toggleAdminMenu();
      return false;
    }
