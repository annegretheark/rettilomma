...
(function(){
  ...
  window.handOpprettKundeDirekte = async function(){
    ...
    try {
      ...
      if (firmaRad && firmaRad.id) {
        await upsertHandFirmaBrukerAdmin(firmaRad, navn, epost);
        try {
      ...
      msg(viaFn
        ...
        : 'Firma/kunde er opprettet, og adminrad er lagt i hand_firma_bruker/hand_ansatt. Hvis kunden fortsatt ikke får logget inn, mangler Auth-brukeren. Da må Edge Function opprett-hand-kunde eller Supabase invite være aktiv.'
      );
  ...
  async function upsertHandFirmaBrukerAdmin(firmaRad, navn, epost){
    if(!window.supabaseClient || !firmaRad || !firmaRad.id || !epost) return;
    const rad = { firma_id: firmaRad.id, epost: epost, rolle: 'admin' };
    try{
      const finnes = await window.supabaseClient.from('hand_firma_bruker').select('id').eq('firma_id', firmaRad.id).eq('epost', epost).limit(1);
      if(!finnes.error && Array.isArray(finnes.data) && finnes.data.length){
        await window.supabaseClient.from('hand_firma_bruker').update(rad).eq('id', finnes.data[0].id);
      } else {
        await window.supabaseClient.from('hand_firma_bruker').insert([rad]);
      }
    }catch(e){ console.warn('Kunne ikke opprette eier/admin i hand_firma_bruker:', e); }
  }
  ...
  window.handSendPassordopprettingDirekte = async function(){ if(window.__handInviteFinalFix20260625 && window.handSendPassordopprettingDirekte.__final !== true){ return window.handSendPassordopprettingDirekte(); } const epost = val('nyHandKundeEpost').toLowerCase(); if(!epost){ msg('Skriv e-post først.', true); return; } msg('Bruker invite via Edge Function. Vent...'); const navn = val('nyHandKundeNavn'); const slug = slugify(navn); const payload = {firmanavn:navn, navn:navn, kontakt_navn:navn, epost:epost, email:epost, telefon:val('nyHandKundeTelefon'), adresse:val('nyHandKundeAdresse'), orgnr:val('nyHandKundeOrgNr'), linknavn:slug, redirectTo:rootBase()+'reset.html'}; try{ const {data,error}=await window.supabaseClient.functions.invoke('opprett-hand-kunde',{body:payload}); if(error) throw error; if(data && data.error) throw new Error(data.error); msg('Invitasjon sendt.'); }catch(e){ msg('Feil: '+(e.message||e), true); } };
