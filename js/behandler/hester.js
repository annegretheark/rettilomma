...
function hentValgtKundeId() {
  return String(document.getElementById("behandlingKunde")?.value || "");
}
...
async function lagreHest() {
  ...
  if (typeof window.fyllBehandlingHesterForValgtKunde === "function") {
    await window.fyllBehandlingHesterForValgtKunde();
  }
}
...
function fyllHestSelect(hester) {
  const select = document.getElementById("behandlingHest");
  if (!select) return;
