console.log("beh-faktura.js lastet");
...
async function lagBehFaktura() {
  ...
  try {
    ...
    fakturaMelding("Henter kunde og ufakturerte behandlinger...");
    ...
    const behandlingRes = await supabaseClient
      .from("beh_behandlinger")
      .select("*, hester:beh_hester(navn,kunde_id)")
      .or("fakturert.is.false,fakturert.is.null")
      .order("dato", { ascending: true });

    if (behandlingRes.error) {
      fakturaMelding(behandlingRes.error.message, true);
      return;
    }

    const alleBehandlinger = behandlingRes.data || [];

    const behandlinger = alleBehandlinger.filter(j =>
      String(j.kunde_id || "") === String(kundeId) ||
      String(j.hester?.kunde_id || "") === String(kundeId)
    );

    if (!behandlinger.length) {
      fakturaMelding("Ingen ufakturerte behandlinger funnet på valgt kunde. Sjekk at behandlingene ligger på samme kunde/eier som du valgte i faktura.", true);
      return;
    }

    const manglerKundeId = behandlinger.filter(j =>
      !j.kunde_id &&
      j.hester?.kunde_id &&
      String(j.hester.kunde_id) === String(kundeId)
    );
    ...
    const eksMva = behandlinger.reduce((sum, j) => {
      return sum +
    ...
    for (const j of behandlinger) {
      ...
      doc.text(`${hestNavn} - ${j.behandlingtype || "Behandling"}`, 45, y);
    ...
    const ids = behandlinger.map(j => j.id);
    ...
    console.error("Feil i lagBehFaktura:", e);
