...
/* RIL 20260626D: Stabil Admin-meny og Min bil under Admin.
   Ingen egen Biler-knapp i topplinjen. Knappen i Admin bruker original visBilerSide-logikk. */
(function(){
  if (window.__rilAdminMinBilStabil) return;
  window.__rilAdminMinBilStabil = true;
  function closeAdminMenu(){
    document.querySelectorAll('#adminMenyGruppe,.meny-gruppe').forEach(function(g){
      g.classList.remove('apen','open');
    });
    var b = document.getElementById('adminMenyKnapp');
    if (b) b.setAttribute('aria-expanded','false');
  }
  function normalizeTopbar(){
    var old = document.getElementById('visAdminBilerKnapp');
    if (old && old.id !== 'visBilerKnapp') old.id = 'visBilerKnapp';
    var b = document.getElementById('visBilerKnapp');
    if (b) {
      b.type = 'button';
      b.textContent = 'Min bil / fyll lager';
      b.classList.remove('hidden','skjult');
      b.style.display = '';
      b.removeAttribute('onclick');
    }
    var topButtons = Array.prototype.slice.call(document.querySelectorAll('.topplinje > #visBilerKnapp'));
    topButtons.forEach(function(btn){
      if (!btn.closest('#adminMenyPanel')) btn.remove();
    });
  }
  document.addEventListener('click', function(ev){
  ...
  window.addEventListener('load', function(){ setTimeout(normalizeTopbar, 0); setTimeout(normalizeTopbar, 500); });
})();
