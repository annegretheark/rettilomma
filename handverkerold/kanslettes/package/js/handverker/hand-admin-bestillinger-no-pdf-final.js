...
(function(){
  ...
  function tick(){
    ...
    var h=low(($('handSideOverskrift')||{}).textContent||'');
    var bil=$('bilerSide');
    if (bil && getComputedStyle(bil).display !== 'none' && (h.indexOf('min bil')>=0 || low(document.body.innerText).indexOf('send bestilling til admin')>=0)) {
      // Hvis gammel kode åpner fyll-bil for admin, send admin tilbake til bestillinger.
      openOrders();
    }
  }
