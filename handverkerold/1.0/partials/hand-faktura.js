...
function hentProsjektTekst(time) {
  ...
  if (!prosjekt) return "";
  return `${prosjekt.prosjektnr || prosjekt.prosjekt_nr || ""} ${prosjekt.navn || ""}`.trim();
}

function tallFaktura(v) {
  const n = Number(String(v == null ? '' : v).replace(',', '.'));
  return Number.isFinite(n) ? n : 0;
}

function prosjektFakturatype(prosjekt) {
  const v = String(prosjekt?.fakturatype || prosjekt?.faktura_type || prosjekt?.pristype || prosjekt?.type_fakturering || '').toLowerCase();
  return v === 'fastpris' ? 'fastpris' : 'time';
}

function prosjektFastpris(prosjekt) {
  return tallFaktura(prosjekt?.fastpris ?? prosjekt?.fast_pris ?? prosjekt?.prosjekt_fastpris ?? prosjekt?.faktura_fastpris);
}
...
function byggFakturaLinjer(timerListe, vareMap) {
  const linjer = [];
  const prosjekt = (timerListe && timerListe.length) ? finnProsjektForTime(timerListe[0]) : null;
  const erFastpris = prosjekt && prosjektFakturatype(prosjekt) === "fastpris";
  const fastpris = erFastpris ? prosjektFastpris(prosjekt) : 0;

  if (erFastpris && fastpris > 0) {
    const t0 = (timerListe || [])[0] || {};
    const prosjektTekst = hentProsjektTekst(t0) || "Fastprisprosjekt";
    linjer.push({
      dato: t0.dato || "",
      beskrivelse: String(prosjektTekst + " - fastpris").slice(0, 40),
      utforer: "",
      antall: 1,
      sumEksMva: fastpris,
      mva: fastpris * MVA_SATS
    });
  }
