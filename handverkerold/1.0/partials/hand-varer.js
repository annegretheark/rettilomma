...
(function () {
  ...
  async function hentFirmaIdForVareImport() {
    if (typeof window.hentInnloggetFirmaIdForBiler === "function") {
      const fid = await window.hentInnloggetFirmaIdForBiler();
      if (fid) return fid;
    }
    ...
    if (userId) {
      const r = await supabaseClient
        ...
        .eq("user_id", userId)
        .in("rolle", ["eier", "admin", "administrator", "faktura"])
        .limit(1)
        .maybeSingle();
  ...
  async function lagreImportRad(rad) {
    const firmaId = rad && rad.firma_id;
    if (!firmaId) return { error: { message: "Mangler firma_id på varen." } };
  ...
  async function importerVarer(inputId = "importVarerFil") {
    ...
    let firmaId = "";
    try {
      firmaId = await hentFirmaIdForVareImport();
    } catch (e) {
      melding("Import stoppet: " + (e.message || String(e)), true);
      return false;
    }
    rader.forEach(rad => { rad.firma_id = firmaId; });
