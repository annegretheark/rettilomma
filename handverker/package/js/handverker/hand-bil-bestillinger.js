...
(function(){
  ...
  async function handLagreBilBestilling(rad){
    ...
    const {error} = await window.supabaseClient.from('hand_bil_bestilling').insert([payload]);
  ...
  async function handLastBilBestillinger(){
    ...
    try{
      let q = window.supabaseClient.from('hand_bil_bestilling').select('*').order('opprettet', {ascending:false}).limit(300);
  ...
  async function handSettBilBestillingStatus(id, status){
    ...
    const {error} = await window.supabaseClient.from('hand_bil_bestilling').update({status}).eq('id', id);
