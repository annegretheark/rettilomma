...
(function(){
  ...
  async function safeUpsertAnsatt(firma, navn, epost){
    ...
    try{ var ex=await c.from('hand_ansatt').select('id').eq('epost',epost).limit(1); if(ex && !ex.error && ex.data && ex.data.length){ await adaptiveWrite('update','hand_ansatt',base,ex.data[0].id); return; } }catch(e){}
