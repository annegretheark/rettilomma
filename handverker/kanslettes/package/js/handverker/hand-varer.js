...
(function () {
  ...
  async function hentVareFirmaTilgang() {
    if (window.handVareFirmaTilgang && window.handVareFirmaTilgang.firma_id) return window.handVareFirmaTilgang;
    ...
    const epost = String(user.email || window.innloggetEpost || localStorage.getItem("handInnloggetEpost") || localStorage.getItem("innloggetEpost") || "").trim().toLowerCase();
    let rad = null;

    if (uid) {
      const r = await supabaseClient.from("hand_firma_bruker").select("firma_id,user_id,epost,rolle").eq("user_id", uid).limit(1).maybeSingle();
      if (!r.error && r.data && r.data.firma_id) rad = r.data;
    }
    if (!rad && epost) {
      const r = await supabaseClient.from("hand_firma_bruker").select("firma_id,user_id,epost,rolle").ilike("epost", epost).limit(1).maybeSingle();
      if (!r.error && r.data && r.data.firma_id) rad = r.data;
    }

    if (!rad || !rad.firma_id) {
      throw new Error("Brukeren mangler kobling i hand_firma_bruker. Legg inn firma_id, user_id, epost og rolle=eier.");
    }

    const rolle = String(rad.rolle || "").trim().toLowerCase();
    const erAdmin = ["eier", "admin", "administrator", "firmaeier", "owner", "sysadm", "sysadmin", "systemadmin"].includes(rolle);
    if (!erAdmin) {
      throw new Error("Brukeren er koblet til firma, men rolle er " + (rolle || "tom") + ". Sett rolle til eier eller admin i hand_firma_bruker.");
    }

    const tilgang = { firma_id: rad.firma_id, rolle, user_id: uid, epost };
    window.handVareFirmaTilgang = tilgang;
    window.aktivFirmaId = rad.firma_id;
    window.handFirmaId = rad.firma_id;
    try {
      localStorage.setItem("aktivFirmaId", rad.firma_id);
      localStorage.setItem("handFirmaId", rad.firma_id);
      localStorage.setItem("firma_id", rad.firma_id);
      localStorage.setItem("firmaId", rad.firma_id);
    } catch (e) {}
    return tilgang;
  }

  async function hentVareFirmaId() {
    const t = await hentVareFirmaTilgang();
    return t.firma_id;
  }
  ...
  async function lastVarer() {
    ...
    const firmaId = await hentVareFirmaId();
    let res = await supabaseClient.from("hand_vare").select("*").eq("firma_id", firmaId).limit(1000);
  ...
  async function lagreVare() {
    ...
    if (window.redigerVareId) {
      res = await supabaseClient.from("hand_vare").update(rad).eq("id", window.redigerVareId).eq("firma_id", firmaId).select();
    } else {
  ...
  async function lagreImportRad(rad, firmaId) {
    rad.firma_id = firmaId;
  ...
  async function importerVarer(inputId = "importVarerFil") {
    ...
    try {
      ...
      for (const rad of rader) {
        const res = await lagreImportRad(rad, firmaId);
    ...
    melding("Importerte " + lagret + " varer til firmaets hovedlager.");
    if (knapp) { knapp.disabled = false; knapp.textContent = "Importer varer"; }
  ...
  window.hentVareFirmaId = hentVareFirmaId;
