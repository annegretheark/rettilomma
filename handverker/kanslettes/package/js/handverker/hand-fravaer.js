...
(function () {
  ...
  function erAdminModus() {
    // VIKTIG: Ikke stol på gammel/localStorage adminmodus alene.
    // Vanlig bruker kan ha "rilAdminModus=ja" hengende igjen etter testing,
    // eller andre scripts kan sette window.erAdmin feil.
    // Adminrettighet skal avgjøres av faktisk rolle på innlogget bruker.
    const rolle = String(window.innloggetRolle || localStorage.getItem("handInnloggetRolle") || "")
      .trim()
      .toLowerCase();
    return rolle === "admin" || rolle === "sysadmin" || rolle === "systemadmin";
  }
  ...
  async function fyllAnsattValg() {
    ...
    select.innerHTML = admin ? '<option value="">Alle ansatte</option>' : '<option value="">Meg selv</option>';

    // Bare ekte admin skal få liste over alle ansatte.
    if (admin) {
    ...
    if (admin && valgt && Array.from(select.options).some(o => String(o.value) === String(valgt))) {
      select.value = valgt;
    } else if (!admin) {
      select.value = "";
    }
  ...
  function oppdaterFravaerRolleVisning() {
    const admin = erAdminModus();

    document.querySelectorAll("#fravaerSide .admin-only").forEach(el => {
      el.hidden = !admin;
      el.style.display = admin ? "" : "none";
      el.classList.toggle("skjult", !admin);
      el.classList.toggle("hidden", !admin);
      el.setAttribute("aria-hidden", admin ? "false" : "true");
    });

    const info = document.querySelector("#fravaerSide .info");
    if (info) {
      info.textContent = admin
        ? "Ansatte kan registrere fravær og avspasering. Admin kan godkjenne og se alle."
        : "Her ser du bare ditt eget fravær og din egen flexisaldo.";
    }

    const godkjenning = $("fravaerGodkjenning");
    if (!admin && godkjenning) godkjenning.innerHTML = "";
  }
  ...
  async function lastFravaerFlexi() {
    ...
    if (erAdminModus()) tegnGodkjenning(fravaer);
      ...
      const godkjenning = $("fravaerGodkjenning");
      if (godkjenning) godkjenning.innerHTML = "";
    }
  }
