...
function beregnSumEksMvaForTime(time) {
  ...
  // I hand_time betyr:
  // timer = antall timer
  // sum_timer = beløp eks. mva
  // timepris = pris per time
  // Bruk sum_timer når den finnes, ellers timer * timepris.
  if (
    time.sum_timer !== undefined &&
    time.sum_timer !== null &&
    Number(time.sum_timer) > 0
  ) {
    return Number(time.sum_timer);
  }
