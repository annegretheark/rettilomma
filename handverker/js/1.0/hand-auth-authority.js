...
(function(){
  ...
  async function getFirmaBruker(userArg){
    ...
    var row = null;
    // Systembruker skal aldri matches mot feil rad via delt/feil user_id.
    if(email === SYS_EMAIL){
      row = await maybeSingleSafe(c.from(TABLE).select('*').ilike('epost', email));
      if(row){ row.__hand_table = TABLE; return row; }
      return { epost: SYS_EMAIL, user_id: uid, firma_id: '', rolle: 'sysadm', __synthetic_sysadm: true, __hand_table: TABLE };
    }
    if(uid){
  ...
  function storeContext(ctx){
    ...
    if(authEmail === SYS_EMAIL || email === SYS_EMAIL) { email = SYS_EMAIL; rolle = 'sysadm'; }
    var firmaId = s(ctx.firma_id || '');
    var userId = s(ctx.user_id || ctx.auth_user_id || window.innloggetUserId || '');
    var sys = isSysRole(rolle);
  ...
  async function applyAuthoritativeAuth(){
    ...
    if(!row){
      return storeContext({ epost: email, user_id: user && user.id, rolle: email === SYS_EMAIL ? 'sysadm' : 'bruker', firma_id: '' });
    }
    return storeContext({
      epost: email === SYS_EMAIL ? SYS_EMAIL : (row.epost || email),
      auth_email: email,
      user_id: row.user_id || (user && user.id),
      firma_id: row.firma_id || '',
      rolle: row.rolle || (email === SYS_EMAIL ? 'sysadm' : 'bruker'),
      rad: row
    });
  }
