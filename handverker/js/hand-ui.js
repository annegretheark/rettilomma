window.addEventListener("load", function () {
  async function nodLoggInn() {
    ...
    try {
      for (const tabell of ["hand_firma", "hand_kunder", "hand_kunde"]) {
    ...
    } catch (e) { console.warn("Kunne ikke sjekke firmaeier i nødinnlogging:", e); }
...
(function () {
  ...
  function koble() {
    ...
    // FIX 20260628: Ikke overstyr hand-biler.js sin riktige lagring.
    // hand-biler.js legger på firma_id og håndterer firma/RLS. Nødlagring brukes bare hvis hand-biler.js mangler.
    if (lagre) lagre.onclick = function (e) {
      if (e) e.preventDefault();
      if (typeof window.lagreBil === "function" && window.lagreBil !== lagreBilNod) {
        return window.lagreBil();
      }
      return lagreBilNod();
    };
...
(function () {
  ...
  function settInnloggetBrukerVisning() {
    ...
    var erSystembruker = String(epost || '').toLowerCase() === 'greknuts@online.no' || window.erSystemadmin === true;
    if (erSystembruker) {
