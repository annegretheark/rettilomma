...
(function () {
  ...
  async function hentFirmaIdForVarer() {
    const fraMinne = String(
      (window.handFirmaTilgang && window.handFirmaTilgang.firma_id) ||
      window.aktivFirmaId ||
      window.handFirmaId ||
      (window.firmaData && window.firmaData.id) ||
      (window.firma && window.firma.id) ||
    ...
    const epost = user && user.email ? String(user.email).trim() : "";

    async function lagreOgReturner(id) {
      id = String(id || "").trim();
      if (!id) return "";
    ...
    // Viktig: hent firma fra hand_firma_bruker først. Det er denne tabellen som gir eier/admin tilgang.
    try {
      if (userId) {
        const r = await supabaseClient
          ...
          .limit(1)
          .maybeSingle();
        if (!r.error && r.data && r.data.firma_id) return await lagreOgReturner(r.data.firma_id);
      }
    } catch (e) {}
    ...
    if (fraMinne) return await lagreOgReturner(fraMinne);
    throw new Error("Fant ikke firma_id for innlogget bruker i hand_firma_bruker.");
  }
  ...
  async function lastVarer() {
    ...
    let firmaId = "";
    try { firmaId = await hentFirmaIdForVarer(); } catch (e) { console.warn(e); }

    let query = supabaseClient.from("hand_vare").select("*").limit(1000);
  ...
  async function lagreVare() {
    ...
    let firmaId = "";
    try { firmaId = await hentFirmaIdForVarer(); }
    catch (e) { melding(e.message || String(e), true); return; }
  ...
  async function lagreImportRad(rad) {
    const firmaId = await hentFirmaIdForVarer();
  ...
  window.hentFirmaIdForVarer = hentFirmaIdForVarer;
