...
(function(){
  ...
  function applyFlags(role, opts){
    opts = opts || {};
    role = norm(role || (opts.allowStoredRole ? getStoredRole() : '') || 'bruker');
    // 20260706: SysAdm skal bare låses når rollen er verifisert fra hand_sysadmin/handsysadmin/hand_sysadm.
    // Gammel localStorage må aldri kunne gjøre en vanlig bruker til sysadm.
    if(lockedSysadm && !isSysRole(role) && opts.force !== true) role = 'sysadm';
    if(isSysRole(role) && opts.verifiedSysadm === true) { role = 'sysadm'; lockedSysadm = true; }
    if(isSysRole(role) && opts.verifiedSysadm !== true && opts.force !== true) role = 'admin';

    const sys = isSysRole(role) && (lockedSysadm || opts.verifiedSysadm === true || opts.force === true);
    ...
    try{
      ...
      else { localStorage.removeItem('rilSysadminModus'); localStorage.removeItem('rilSysadminRettighet'); }
    }catch(e){}
  ...
  function installProtectedProps(){
    try{
      ...
      // Ikke lås SysAdm fra gamle localStorage/window-verdier. Bare tabellvakten kan verifisere sysadm.
      if(isSysRole(current)) { lockedSysadm = false; roleValue = 'bruker'; }
  ...
  // Ikke bruk lagret/metadatarolle til sysadm. hand-sysadm-authority/table-only-guard verifiserer mot SysAdm-tabell.
  if(isSysRole(getStoredRole())) applyFlags('bruker', { clearSysMode:true });
  setTimeout(function(){ hentAuthRolle().then(function(r){ if(isSysRole(r)) applyFlags('admin', { clearSysMode:true }); }); }, 0);
