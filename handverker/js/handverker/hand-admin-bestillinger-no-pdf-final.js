...
(function(){
  ...
  function doneStatus(st){ return /^(admin_besvart|godkjent|besvart|levert|ferdig|mottatt|avsluttet|utlevert|fullfort|fullført|delvis_mottatt|arkivert|lukket)$/.test(low(st)); }
  function answeredStatus(st){ return /^(admin_besvart|rest_hos_admin|restordre_admin|restordre|delvis_admin|delvis_levert)$/.test(low(st)); }
  function doneForAdmin(r){ return !!(r && (r.arkivert === true || r.lukket === true || r.lagt_pa_bil === true || r.ansatt_godkjent === true || doneStatus(r.status) || answeredStatus(r.status) || r.admin_svar_sendt === true)); }
  ...
  async function loadRows(){
    ...
    var q = c.from('hand_bil_bestilling').select('*').limit(800);
    var res = await q;
  ...
  function renderRows(rows){
    ...
    active.forEach(function(lines){
      ...
      lines.forEach(function(r){
        ...
        var best = Math.max(0, n(r.bestilt || r.antall || r.rest));
  ...
  function linjePayloadFraInput(line){
    ...
    return {
      ...
      payload: {
        ...
        levert: nyLevert,
        rest: rest,
        status: god <= 0 ? 'restordre' : (rest > 0 ? 'rest_hos_admin' : 'admin_besvart'),
        admin_svar_sendt: true,
        admin_svar_tid: new Date().toISOString(),
        godkjent_dato: new Date().toISOString(),
        admin_godkjent: true,
        kommentar: rest > 0 ? 'Admin godkjente ' + god + ', rest ' + rest : 'Admin godkjente alt'
      }
    };
  }
