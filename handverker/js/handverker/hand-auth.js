...
async function loggInn() {
  ...
  // 20260706: Etter all gammel/fallback-logikk låses rolle/firma til public.hand_firma_bruker.
  // Dette hindrer at firma_brukere eller hand_ansatt overskriver sysadm/admin.
  if (typeof window.handApplyAuthoritativeAuth === "function") {
    try {
      const handCtx = await window.handApplyAuthoritativeAuth();
      rolle = handCtx.rolle || rolle || "bruker";
      erAdmin = !!handCtx.admin;
      window.erAdmin = !!handCtx.admin;
      window.erSystemadmin = !!handCtx.sysadm;
      if (handCtx.firma_id) {
        window.aktivFirmaId = handCtx.firma_id;
      }
    } catch (e) {
      console.warn("Kunne ikke låse autoritativ rolle fra hand_firma_bruker:", e);
    }
  }
...
async function loggUt(event) {
  if (typeof window.handLoggUtSikkert === "function") return window.handLoggUtSikkert(event);
