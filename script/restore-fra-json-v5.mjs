...
function preprocessRows(table, rows) {
  return rows.map(row => {
    const out = { ...row };

    // I denne backupen er firma.id = 1, men dagens hov_firma.id er UUID.
    // Da må databasen få lage ny id selv.
    if (table === 'hov_firma') {
      delete out.id;
      delete out.org_nr;
    }
...
async function upsertBatch(table, rows, token) {
  rows = preprocessRows(table, rows);
...
async function main() {
  console.log('Hovslager JSON-gjenoppretting v5');
