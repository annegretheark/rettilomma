...
Deno.serve(async (req) => {
  ...
  if (!isCron) {
    ...
    const erAdmin = email === "greknuts@online.no" || rolle === "admin" || rolle === "systemadmin";
  ...
  const backup: Record<string, unknown> = {
    type: "rett-i-lomma-veterinaer-auto-backup",
    versjon: 1,
  ...
  const path = `${isCron ? "auto" : "manual"}/${dato}/vet-backup-${stamp}.json`;
