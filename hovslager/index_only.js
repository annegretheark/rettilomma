...
(function(){
  ...
  function renderJobbBildePreview(items){
    ...
    el.innerHTML = bilder.length ? bilder.map((x,idx)=>{
      ...
      return `<figure class="timeline-photo"><button type="button" class="delete-photo-btn" ${delAttrs} title="Slett bilde">🗑</button><img class="thumb clickable-img" src="${esc(x.url)}" data-full-src="${esc(x.url)}" alt="Jobb-bilde" title="Klikk for større bilde"><figcaption>${dato}${pending}</figcaption></figure>`;
    }).join('') : '<span class="muted">Ingen bilder valgt.</span>';
  }
  function renderCombinedJobbBildePreview(){
    const pending=(app.pendingJobbFiles||[]).map(x=>({url:x.url, dato:x.dato, pending:true}));
  ...
  function bind(){
    ...
    $('navReadLastJobbBtn')?.addEventListener('click', openNewJobbWindowFromNav);
    ...
    document.querySelectorAll('[data-tab]').forEach(b=>b.addEventListener('click',()=>{ showTab(b.dataset.tab); if(b.dataset.tab==='jobber') setTimeout(()=>{ const list=$('jobbList'); if(list) list.scrollIntoView({behavior:'smooth', block:'start'}); }, 50); }));
  ...
  function renderAdmin(){
    ...
    if(firmaList){
      firmaList.innerHTML = `<div class="msg">Antall lagrede firmaer vist: ${firmaer.length}. Hvis du forventer flere, må de finnes i Supabase-tabellen <code>hov_firma</code> og SysAdm-brukeren må ha tilgang via RLS.</div>` + table(['Firma / abonnement','E-post','Telefon','Org.nr','ID','Handling'], firmaRows);
  ...
  function startVoiceJobb(){
    ...
    try{
      ...
      rec.onstart = () => {
        ...
        msg('voiceJobbMsg','Lytter ... snakk inn jobben. Trykk Stopp når du er ferdig, eller skriv i feltet og trykk Bruk skrevet tekst.','ok');
      };
      rec.onerror = (ev) => {
