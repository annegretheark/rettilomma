...
function vetSettToppInfoFallback() {
  // Hindrer at toppfeltet blir stående på "Laster rolle ..." hvis brukerdata
  // eller klinikkkobling ikke er ferdig satt når siden tegnes.
  const rolleEl = document.getElementById("vetToppRolle");
  if (rolleEl && String(rolleEl.textContent || "").toLowerCase().includes("laster")) {
    rolleEl.textContent = vetRolleVisningsnavn();
  }
  const navnEl = document.getElementById("vetToppNavn");
  if (navnEl && (!String(navnEl.textContent || "").trim() || String(navnEl.textContent || "").toLowerCase().includes("veterinær"))) {
    navnEl.textContent = String(vetInnloggetBrukerNavn || vetInnloggetEpost || "Veterinær").trim();
  }
}
