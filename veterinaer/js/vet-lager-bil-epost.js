...
async function lastVetFakturaer() {
  // VIKTIG: fakturaer er en felles tabell som også kan inneholde veterinær/hovslager/testdata.
...
function tegnMinBilFyllListe() {
  ...
  if (!bilId) {
    liste.innerHTML = "";
    if (info) {
      info.textContent = vetBiler.length
        ? "Ingen bil er koblet automatisk til deg. Velg bil manuelt i listen, eller be admin koble bilen til navnet/e-posten din."
        : "Ingen biler er tilgjengelige for denne brukeren. Sjekk at brukeren er koblet til riktig klinikk, at bilen er aktiv, og at vanlig bruker har lesetilgang til vet_biler/vet_bil_lager.";
    }
    return;
  }
